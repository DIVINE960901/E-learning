<script  setup>
import { ref } from 'vue'
import { addCourseService,courseDetailService,deleteCourseService } from '@/api/personPage.js'
...
const courseList = async()=>{
    let result = await courseDetailService()
...
//     console.log(file)
//     uploadImg.value.removeFile(file);
//     uploadImg.value.clearFiles();
        ...
        <el-form label-position="top" :model="postData">
            ...
            <el-form-item label="课程封面图">
                <el-upload ref="uploadImg" action="http://localhost:8080/uploadImage/upload" list-type="picture-card"
                    :auto-upload="false" :on-exceed="handleExceed" :limit="1" :on-success="imageUploadSuccess" :accept="'.jpg, .jpeg, .png'">
                    <el-icon>
                        <Plus />
                    </el-icon>
                    <template #file="{ file }">
            ...
            <el-form-item label="上传视频">
                <el-upload ref="uploadRef" class="upload-demo" action="http://localhost:8080/vod/upload"
                    :auto-upload="false" :on-success="handleVodUploadSuccess" :accept="'.mp4'">
                    <template #trigger>
